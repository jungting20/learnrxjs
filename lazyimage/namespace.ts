...
import { map, mergeMap, tap, switchMap, mergeAll, bufferCount, toArray, flatMap, concatMap, concatAll } from "rxjs/operators";
import { Image } from "./interfaces";



export namespace $ {
  export const el = (str: string) => {
    const div = document.createElement('div');
    div.innerHTML = str;
    return div.children[0];
  }
  export const all = (str: string) => document.querySelectorAll(str);
}
...
export namespace Images {
  ...
  //음 뺄필요없었을듯...
  const limit = (count:number) => ($input:Observable<any>) => $input.pipe(
    bufferCount(count),
    concatMap((imgarr:HTMLImageElement[]) => from(imgarr).pipe(
      tap(show)
    ))
  )


  const show_images = (count:number = 0) => {
    return function all_show(imgs: NodeList) {
      return from<NodeList>(imgs)
        .pipe(
          //90개의 옵저버블....
          //concatMap =>L.map , mergeMap => c.map
          concatMap((img: HTMLImageElement) => new Promise(resolve => {
  ...
  export function show(onloaded_img: HTMLImageElement) {
    onloaded_img.classList.add('fade-in');
    onloaded_img.classList.remove('fade');
    return onloaded_img;
  }

  export const fetch = () => new Promise(resolve => setTimeout(() => resolve([
  ...
  export const tmpl = (imgs: Image[]): string => `<div class="images">
      ${imgs.map((img: Image) => `
      <div class="image">
  ...
  export const loader = show_images(4);
